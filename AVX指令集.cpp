...
const int N =1200 ; // Define the size of the matrix
...
void gaussian_elimination(float *A, int n) {
    ...
    float *x = new float[n]; // Result vector x
    x[n - 1] = 0.0; // Initialize the last element of x

    for (int i = n - 1; i >= 0; --i) {
        float sum = 0.0;
        for (int j = i + 1; j < n; ++j) {
            sum += A[i * n + j] * x[j]; // Calculate sum
        }
        x[i] = (A[i * n + n] - sum) / A[i * n + i]; // Solve for x[i]
    }

    delete[] x; // Free memory allocated for x
}
...
void gaussian_elimination_division(float *A, int n) {
    ...
    for (int k = 0; k < n; ++k) {
        __m256 vt = _mm256_set1_ps(A[k * n + k]); // Load diagonal element
        for (int j = k + 1; j < n; j += 16) { // 8-way vectorization
            __m256 va = _mm256_loadu_ps(&A[k * n + j]); // Load 8 consecutive elements
            va = _mm256_div_ps(va, vt); // Vectorized division
            _mm256_storeu_ps(&A[k * n + j], va); // Store the result back to memory
        }
...
void gaussian_elimination_elimination(float *A, int n) {
    ...
    for (int k = 0; k < n; ++k) {
        ...
        for (int i = k + 1; i < n; ++i) {
            __m256 vaik = _mm256_set1_ps(A[i * n + k]); // Load element A[i][k]
            for (int j = k + 1; j < n; j += 16) { // 8-way vectorization
                __m256 vakj = _mm256_loadu_ps(&A[k * n + j]); // Load 8 consecutive elements from A[k][j]
                __m256 vaij = _mm256_loadu_ps(&A[i * n + j]); // Load 8 consecutive elements from A[i][j]
                __m256 vx = _mm256_mul_ps(vakj, vaik); // Vectorized multiplication
                vaij = _mm256_sub_ps(vaij, vx); // Vectorized subtraction
                _mm256_storeu_ps(&A[i * n + j], vaij); // Store the result back to memory
            }
            A[i * n + k] = 0.0; // Set A[i][k] to zero
        }
    }
